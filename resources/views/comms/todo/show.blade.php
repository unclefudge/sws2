            ...
            <div class="col-md-12">
                <div class="portlet light bordered">
                    ...
                    <div class="portlet-body form">
                        <form method="POST" action="{{ action([App\Http\Controllers\Comms\TodoController::class, 'update'], $todo->id) }}" enctype="multipart/form-data" id="todo_form">
                            @csrf
                            @method('PATCH')
                            @include('form-error')

                            <x-form.hidden name="status" :value="$todo->status"/>
                            <x-form.hidden name="delete_attachment" :value="0"/>
                            ...
                            <div class="form-body">
                                ...
                                <div class="row">
                                    <div class="col-md-8">
                                        <x-form.input name="name" label="Name" :value="$todo->name" :readonly="!($todo->status && Auth::user()->id == $todo->created_by)"/>
                                    </div>
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        @else
                                            <x-form.input name="s_due_at" label="Due Date" :value="($todo->due_at) ? $todo->due_at->format('d/m/Y') : 'none'" readonly/>
                                        @endif
                                    </div>
                                </div>
                                ...
                                @if ($todo->status && Auth::user()->hasAnyRole2('whs-manager|mgt-general-manager|web-admin') && !in_array($todo->type, ['equipment']))
                                    <div class="row">
                                        ...
                                        <div class="col-md-3">
                                            <x-form.datepicker name="completed_at" label="Completed Date (optional)" :value="$todo->completed_at ? $todo->completed_at->format('d/m/Y') : null" format="dd/mm/yyyy" clear-button wrapper-class="input-medium" readonly/>
                                        </div>
                                    </div>
                                @endif
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <x-form.textarea name="info" label="Description of what to do" :value="$todo->info" rows="4" :readonly="!($todo->status && Auth::user()->id == $todo->created_by)"/>
                                    </div>
                                    ...
                                    <div class="col-md-12">
                                        @if ($todo->status)
                                            <x-form.textarea name="comments" label="Comments" :value="$todo->comments" rows="4"/>
                                        @else
                                            <x-form.textarea name="s_comments" label="Comments" :value="$todo->comments" rows="4" readonly/>
                                        @endif

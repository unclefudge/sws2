<div>
    ...
    @once
        <style>
            ...
            .prac-items-table tbody tr:nth-child(odd) > td { background: #fff !important; }
    ...
    @if ($allDone && $prac->status)
        <div class="note note-warning">
            <p style="margin:0">All items have been completed and request requires <button type="button" class="btn btn-xs btn-outline dark disabled">Sign Off</button> at the bottom.</p>
        </div>
    @endif

    <h4 class="clearfix" style="margin-bottom:5px">
        Prac Completion Items
                ...
                <tr wire:key="prac-item-{{ $item->id }}" data-item-id="{{ $item->id }}" @if ($canEdit && $filter === 'all') x-bind:style="String(draggingId) === '{{ $item->id }}' ? 'opacity:.45' : ''" @endif>
                    ...
                    <td style="padding-top:15px">
                        ...
                        <div x-data="{ expanded: false }">
                            <span x-show="!expanded">{{ $itemPreview }}</span>
                            <span x-show="expanded" x-cloak>{{ $item->name }}</span>
                            @if (mb_strlen($item->name) > $previewLength)
                                <a href="#" x-on:click.prevent="expanded = !expanded" x-text="expanded ? 'Show less' : 'Show more'" style="margin-left:5px"></a>
                            @endif
                        </div>
                    </td>
                    <td style="padding-top:15px">
                    ...
                    <td>
                        @if ($item->sign_by)
                            ...
                            {{ $users->get($item->sign_by)?->full_name ?? 'Unknown' }}
                        @else
                            -
                        @endif
                    </td>
    ...
    @else
        <div class="font-grey-silver">{{ $filter === 'all' ? 'No practical completion items.' : 'No ' . $filter . ' practical completion items.' }}</div>
    @endif

    <x-ui.modal :show="$showAddModal" title="Add Prac Completion Item" close-action="closeModals">
        <div class="form-group {{ $errors->has('itemName') ? 'has-error' : '' }}">
            <label class="control-label">Item</label>
            <textarea wire:model="itemName" rows="3" class="form-control" placeholder="Specific details of practical completion item"></textarea>
    ...
    <x-ui.modal :show="$showEditModal" title="Edit Prac Completion Item" close-action="closeModals">
        @php($itemPreview = \Illuminate\Support\Str::limit($itemName, $previewLength))
        <div wire:key="prac-item-summary-{{ $editingItemId }}" x-data="{ expanded: false }" style="margin-bottom:18px">
            <b>Item</b><br>
            <span x-show="!expanded">{{ $itemPreview }}</span>
            <span x-show="expanded" x-cloak>{{ $itemName }}</span>
            @if (mb_strlen($itemName) > $previewLength)
                <a href="#" x-on:click.prevent="expanded = !expanded" x-text="expanded ? 'Show less' : 'Show more'" style="margin-left:5px"></a>
            @endif
        </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="form-group {{ $errors->has('assignedTo') ? 'has-error' : '' }}">
                    ...
                    <div wire:ignore wire:key="prac-assigned-to-{{ $editingItemId }}">
                        <select class="form-control bs-select" data-width="100%" data-live-search="true" data-container="body" data-size="7" x-init="if (!$($el).parent().hasClass('bootstrap-select')) $($el).selectpicker()" x-on:change="$wire.set('assignedTo', $el.value)">
                            <option value="" {{ $assignedTo === '' ? 'selected' : '' }}>Select company</option>
                            @foreach ($companyOptions as $companyId => $companyName)
                                <option value="{{ $companyId }}" {{ (string)$assignedTo === (string)$companyId ? 'selected' : '' }}>{{ $companyName }}</option>
                            @endforeach
                        </select>
                    </div>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group {{ $errors->has('plannerTaskId') ? 'has-error' : '' }}">
                    ...
                    <div wire:ignore wire:key="prac-planner-task-{{ $editingItemId }}-{{ $assignedTo }}">
                        <select class="form-control bs-select" data-width="100%" data-live-search="true" data-container="body" data-size="8" {{ !$assignedTo ? 'disabled' : '' }} x-init="if (!$($el).parent().hasClass('bootstrap-select')) $($el).selectpicker()" x-on:change="$wire.set('plannerTaskId', $el.value)">
                            <option value="" {{ $plannerTaskId === '' ? 'selected' : '' }}>{{ $assignedTo ? 'Select task' : 'Select company first' }}</option>
                            @foreach ($plannerTaskOptions as $taskId => $taskName)
                                <option value="{{ $taskId }}" {{ (string)$plannerTaskId === (string)$taskId ? 'selected' : '' }}>{{ $taskName }}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="col-md-6">
                @if ($plannerTaskId)
                    <x-form.datepicker name="pracPlannerDate" label="Task Date" :value="$plannerDate" format="dd/mm/yyyy" readonly wire:ignore
                        x-init="
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group {{ $errors->has('itemStatus') ? 'has-error' : '' }}">
                    ...
                    <div wire:ignore wire:key="prac-item-status-{{ $editingItemId }}">
                        <select class="form-control bs-select" data-width="100%" data-container="body" x-init="if (!$($el).parent().hasClass('bootstrap-select')) $($el).selectpicker()" x-on:change="$wire.set('itemStatus', $el.value)">
                            <option value="1" {{ (string)$itemStatus === '1' ? 'selected' : '' }}>Incomplete</option>
                            <option value="0" {{ (string)$itemStatus === '0' ? 'selected' : '' }}>Completed</option>
                        </select>
                    </div>
    ...
    <x-ui.confirm-modal :show="$showDeleteModal" close-action="closeModals" confirm-action="deleteItem" confirm-label="Yes, delete it">
        ...
        <div>This practical completion item will be permanently deleted.</div>

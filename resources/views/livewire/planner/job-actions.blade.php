    ...
    @if ($showMenu && $canEdit)
        <div class="planner-job-menu" x-data="{ open: false }" x-on:click.outside="open = false">
            ...
            <div class="planner-job-dropdown" x-show="open" x-cloak>
                @if ($canManage)
                    <button type="button" wire:click="openAction('add')" x-on:click="open = false">Add Job Start</button>
                @endif
                <button type="button" wire:click="openAction('move')" x-on:click="open = false">Move Job Start</button>
                @if ($canManage)
                    <button type="button" wire:click="openAction('allocate')" x-on:click="open = false">Allocate Job</button>
                @endif
            </div>
        </div>
    @endif
    ...
    <x-ui.modal :show="$showModal" :title="$action === 'add' ? 'Add Job Start' : ($action === 'move' ? 'Move Job Start' : 'Allocate Job')" close-action="closeModal" max-width="820px" class="planner-job-action-modal">
        @if ($showModal)
            ...
            <div wire:ignore wire:key="planner-job-site-{{ $action }}-{{ count($siteOptions) }}">
                <x-form.select name="plannerJobSite" label="Site" placeholder="Select site" :value="$selectedSiteId" data-width="100%" data-live-search="true" data-container="body" data-size="8" x-init="if ($.fn.selectpicker && !$($el).parent().hasClass('bootstrap-select')) $($el).selectpicker()"
                               x-on:change="$wire.set('selectedSiteId', $el.value)">
            ...
            @if (in_array($action, ['add', 'move'], true))
                <div wire:ignore wire:key="planner-job-date-{{ $action }}-{{ $jobDate }}" x-data x-init="
                    ...
                    <div class="input-group date planner-job-datepicker" x-ref="picker"><input type="text" id="plannerJobDate" class="form-control" placeholder="Select date" readonly><span class="input-group-btn"><button type="button" class="btn default date-set" aria-label="Choose date"><i
            ...
            @elseif ($action === 'allocate')
                <div wire:ignore wire:key="planner-job-supervisor-{{ count($supervisorOptions) }}">
                    <x-form.select name="plannerJobSupervisor" label="Supervisor" placeholder="Select supervisor" :value="$selectedSupervisorId" data-width="100%" data-live-search="true" data-container="body" data-size="8"
                                   x-init="if ($.fn.selectpicker && !$($el).parent().hasClass('bootstrap-select')) $($el).selectpicker()" x-on:change="$wire.set('selectedSupervisorId', $el.value)">
                        @foreach ($supervisorOptions as $supervisor)
                            <option value="{{ $supervisor['id'] }}" @selected((int)$selectedSupervisorId === (int)$supervisor['id'])>{{ $supervisor['name'] }}</option>
                        @endforeach
                    </x-form.select>
                </div>
            @endif
        ...
        <x-slot name="footer">
            ...
            <button type="button" class="sws-modal-btn sws-modal-btn-primary" wire:click="saveAction" wire:loading.attr="disabled" wire:target="saveAction"><i
                        class="fa {{ $action === 'allocate' ? 'fa-user' : ($action === 'move' ? 'fa-exchange' : 'fa-plus') }}"></i> {{ $action === 'add' ? 'Add Job Start schedule' : ($action === 'move' ? 'Move Job Start' : 'Allocate Job') }}</button>
        </x-slot>
    </x-ui.modal>

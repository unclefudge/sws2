            ...
            <div class="col-md-12">
                <div class="portlet light bordered">
                    ...
                    <div class="portlet-body form">
                        <form method="POST" action="{{ action([App\Http\Controllers\Site\SiteShutdownController::class, 'update'], $shutdown->id) }}" class="horizontal-form">
                            @csrf
                            @method('PATCH')
                            @include('form-error')
                            <div class="form-body">
                                ...
                                @foreach ($shutdown->items as $item)
                                    ...
                                    <div class="row" style="margin-top: 10px; margin-bottom: 10px;">
                                        <div class="col-sm-2">
                                            @if ($item->type == 'yn')
                                                <x-form.select :name="'resp-' . $item->order" :options="['' => 'Select', 'Yes' => 'Yes', 'N/A' => 'N/A']" :value="$item->response"/>
                                            @else
                                                <x-form.select :name="'sel-' . $item->order" :options="['' => 'Select', 'Yes' => 'Yes']" :value="$item->response ? 'Yes' : ''" :id="'sel-' . $item->order"/>
                                            @endif
                                        </div>
                                        <div class="col-sm-10">
                                            <div>{{ $item->name }}</div>
                                            @if ($item->type != 'yn')
                                                <div style="margin-top: 10px">
                                                    <x-form.textarea :name="'resp-' . $item->order" :id="'resp-' . $item->order" :value="$item->response" rows="5" placeholder="Provide details" style="display:none"/>
                                                </div>
                                            @endif
                                        </div>
                                    </div>

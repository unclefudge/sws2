    ...
    <div class="portlet-body">
        @if ($incident->witness->count())
            @foreach ($incident->witness as $witness)
                <div class="row">
                    <div class="col-xs-1"><a href="/site/incident/{{ $incident->id }}/people/{{ $witness->id  }}"><i class="fa fa-search"></i></a></div>
                    <div class="col-xs-10">{{ $witness->name }} @if ($witness->user) ({{ $witness->company->name }}) @endif </div>

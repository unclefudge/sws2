...
@section('content')
    <div class="page-content-inner">
        ...
        <x-ui.bootstrap-modal id="modal_create_category" title="Add category" max-width="560px">
            <form method="POST" action="{{ action([App\Http\Controllers\Site\SiteQaCategoryController::class, 'store']) }}" id="create-category-form">
                @csrf
        ...
        <x-ui.bootstrap-modal id="modal_edit_category" title="Edit category" max-width="560px">
            <form method="POST" action="{{ url('/site/qa/category/0') }}" id="edit-category-form">
                @csrf
                @method('PATCH')
...
<script type="text/javascript">
    ...
    var categoryBaseUrl = @js(url('/site/qa/category'));
    ...
    @if (old('_category_form', session('qa_category_modal')) === 'create')
        $('#modal_create_category').modal('show');
    @elseif (old('_category_form', session('qa_category_modal')) === 'edit')
        openEditCategory(@js(old('_category_id', data_get(session('qa_category'), 'id'))), @js(old('edit_name', data_get(session('qa_category'), 'name'))));
    @endif
</script>
@stop

                ...
                <div class="portlet light bordered">
                    ...
                    <div class="portlet-body form">
                        <form method="POST" action="{{ action([App\Http\Controllers\Site\SiteDocController::class, 'store']) }}" class="horizontal-form" enctype="multipart/form-data">
                            ...
                            <x-form.hidden name="type" value="PLAN" id="type"/>
                                    ...
                                    <div class="col-md-6">
                                        <div id="site_id_form">
                                            @if (!Auth::user()->company->subscription && Auth::user()->company->parent_company == '3')
                                                <x-form.select name="site_id" label="Site" :options="Auth::user()->company->reportsTo()->sitesSelect('prompt', '1')" :value="$site_id ?? ''" plugin="select2" placeholder="Select Site"/>
                                            @else
                                                <x-form.select name="site_id" label="Site" :options="Auth::user()->company->sitesSelect('prompt')" :value="$site_id ?? ''" plugin="select2" placeholder="Select Site"/>
                                            @endif
                                        </div>
                                    </div>

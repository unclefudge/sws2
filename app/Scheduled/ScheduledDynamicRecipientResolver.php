...
class ScheduledDynamicRecipientResolver
{
    public function siteSupervisor(
        ...
        $supervisor = $site && $site->supervisor_id
            ? User::find((int) $site->supervisor_id)
            : null;

        return $this->user($key, $label, $supervisor, $type, $required);
    }

    public function plannerCompanyContact(
        string $key,
        string $label,
        ?SitePlanner $planner,
        string $type = 'cc',
        ...
        $company = $planner && $planner->entity_type === 'c'
            ? Company::find((int) $planner->entity_id)
            : null;
    ...
    public function users(
        ...
        $resolved = collect($users)
            ...
            ->map(fn(User $user) => $this->recipient(
                $key,
                $label,
                $type,
                $user->email,
    ...
    public function todoAssignees(
        ...
        $direct = collect($this->users($key, $label, $assigned, $type, $required))
        ...
        $companies = $assigned->pluck('company_id')->filter()->unique()
            ->map(fn($companyId) => Company::find((int) $companyId))
            ->filter();

        $fallbacks = $companies->flatMap(fn(Company $company) => $this->companyContact(
            $key,
            $label,
            $company,
            $type,
            ...
            ->unique(fn(array $recipient) => $recipient['email'])
            ->values()
            ->all();

        return $fallbacks ?: [
            $this->missing($key, $label, $type, $required, 'No assigned ToDo user or company contact has a valid email.'),
        ];
    }
    ...
    public function companyContact(
        ...
        return [$this->missing(
            ...
            $type,
            $required,
            "{$company->name} has no active primary/secondary contact or valid company email."
        )];
    }

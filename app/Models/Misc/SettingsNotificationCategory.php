...
class SettingsNotificationCategory extends Model
{
    ...
    * Create Select field for Notifications
     ...
     * @return string
     */
    public function notificationSelect(bool $manageCategory = false, bool $canMoveUp = false, bool $canMoveDown = false, string $managementType = 'report')
    {
        $managementPrefix = $managementType === 'notification_group' ? 'notification-group' : 'report';
        ...
        if ($manageCategory) {
            if ($canMoveUp)
                $str .= ' <a href="javascript:;" class="' . $managementPrefix . '-move" data-id="' . $this->id . '" data-direction="up" title="Move up"><i class="fa fa-arrow-up font-grey-silver"></i></a>';

            if ($canMoveDown)
                $str .= ' <a href="javascript:;" class="' . $managementPrefix . '-move" data-id="' . $this->id . '" data-direction="down" title="Move down"><i class="fa fa-arrow-down font-grey-silver"></i></a>';

            if ($this->system)
                $str .= ' <span title="Used by SafeWorkSite code - disable rather than delete"><i class="fa fa-lock font-grey-silver"></i></span>';
            else
                $str .= ' <a href="javascript:;" class="' . $managementPrefix . '-delete" data-id="' . $this->id . '" data-name="' . htmlspecialchars($this->name, ENT_QUOTES, 'UTF-8') . '" title="Delete"><i class="fa fa-trash font-red"></i></a>';
        }

...
class SiteFocItem extends Model
{
    ...
    {
        if ($status)
            return Todo::where('status', $status)->where('type', 'foc_item')->where('type_id', $this->id)->get();

        return Todo::where('type', 'foc_item')->where('type_id', $this->id)->get();
    }

    public function closeToDo($type = 'foc_item')
    {

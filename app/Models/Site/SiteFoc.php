...
class SiteFoc extends Model
{
    ...
    protected $fillable = [
        ...
        'foc_requested', 'final_photos_rcvd', 'portal_fee_paid', 'wbo_waiting', 'notes', 'status', 'created_by', 'updated_by', 'created_at', 'updated_at'];
    protected $casts = ['supervisor_sign_at' => 'datetime', 'manager_sign_at' => 'datetime', 'foc_requested' => 'datetime', 'final_photos_rcvd' => 'date', 'portal_fee_paid' => 'boolean', 'wbo_waiting' => 'boolean',];
    ...
    {
        return $this->hasMany('App\Models\Site\SiteFocItem', 'foc_id');
    }


    /**
     * A Site FOC 'may' have been assigned to a Company.
     */
    public function assignedTo()
    {
        $assigned = SiteFocItem::where('foc_id', $this->id)->pluck('assigned_to')->toArray();
    ...
    {
        $action = Action::create(['action' => 'Moved FOC to On Hold', 'table' => $this->table, 'table_id' => $this->id]);
        $this->status = 4;
        $this->save();
     ...
     */
    public function closeToDo($type = 'foc')
    {
    ...
    {
        $action = Action::create(['action' => 'Moved FOC to Active', 'table' => $this->table, 'table_id' => $this->id]);
        $this->status = 1;
        $this->save();
    ...
    public function createAssignSupervisorToDo($user_list)
    {
        ...
        $todo_request = [
            'type' => 'foc',
            'type_id' => $this->id,
            'name' => 'FOC created - ' . $this->site->name,
    ...
    public function emailAssigned($user)
    {
        ...
        Mail::to($email_to)->send(new \App\Mail\Site\SiteFocAssigned($this));

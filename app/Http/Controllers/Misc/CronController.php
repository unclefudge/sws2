...
class CronController extends Controller
{
    ...
    /*static public function expiredCompanyDoc()
    {
        ...
        $today = Carbon::today();
        $week4_coming = Carbon::today()->addDays(28);
        ...
        $dates = [
            $week4_coming->format('Y-m-d') => "Expiry in 4 weeks on " . $week4_coming->format('d/m/Y'),
                ...
                foreach ($docs as $doc) {
                    ...
                    if ($company->status) {
                        ...
                        if (!$standard_details) {
                            ...
                            // - @ 4 weeks prior also send parent company an email
                            // - @ 2 weeks prior also send parent company an email
                            if ($doc->status) {
...
if (!Auth::check()) file_put_contents($logFile, $log, FILE_APPEND);
    ...
    */
    ...
    /*static public function expiredSWMS()
    {
        ...
        $week2_coming = Carbon::today()->addDays(14);
        $week2_coming2 = Carbon::today()->addDays(14)->subYears(2);
        $week4_ago = Carbon::today()->subDays(28);
        $week4_ago2 = Carbon::today()->subDays(28)->subYears(2);
...
if (!Auth::check()) file_put_contents($logFile, $log, FILE_APPEND);
   ...
   */
    ...
    */
    ...
    /* Overdue toolbox ToDos are handled by App\Scheduled\Reports\OverdueTodoReport. */

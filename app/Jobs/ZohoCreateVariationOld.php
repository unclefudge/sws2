...
class ZohoCreateVariationOld implements ShouldQueue
{
        ...
        try {
            ...
            if ($zohoVariationId) {
                ...
                foreach ($attachments as $attachment) {
                    ...
                    Log::info('Checking SiteNote attachment in FileBank', ['site_note_id' => $note->id, 'attachment_id' => $attachment->id, 'name' => $attachment->name, 'filebank_path' => $fileBankPath, 'filebank_exists' => FileBank::exists($fileBankPath), 'filebank_size' => FileBank::fileSize($fileBankPath),]);
                    ...
                    $files[] = ['name' => $this->safeFilename($attachment->name ?: $attachment->attachment), 'contents' => FileBank::readStream($fileBankPath),];
                }
